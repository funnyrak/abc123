...
3. `supabase/migrations/` 안의 파일을 **번호 순서대로**(0001 → 0012) SQL Editor에 붙여넣고 실행합니다. 뒤 번호일수록 앞 마이그레이션의 테이블/정책에 의존하므로 순서를 지켜야 합니다. `0009_seed_mentors.sql`까지 실행하면 실제 멘토 풀 565명이 바로 채워집니다.
   - 이미 0001~0010까지 실행해서 운영 중인 프로젝트라면, **`0011_mentor_category_arrays.sql`과 `0012_organizations_signup_insert.sql`만 추가로** 실행하면 됩니다.
4. 담당자·학생 가입 시 더 이상 사전 등록된 학교/기관 코드가 필요하지 않습니다 — 학교/기관명을 입력하면 같은 이름의 기관이 있으면 재사용하고, 없으면 자동으로 새로 만듭니다 (Phase 8 참고).
...
- 학생 참여 승인: 기획서(§7.4)는 "학교 코드 입력 → 담당자 승인 → 연결"이지만, 현재는 가입 시 입력한 학교/기관명으로 즉시 `org_id`가 연결(필요하면 기관 자동 생성)되고 담당자 승인 단계는 생략되어 있습니다. 같은 학교라도 오탈자로 이름을 다르게 입력하면 별도 기관으로 생성되므로, 정확한 명칭 안내가 필요합니다.
...
**산업/직무 카테고리 통일 + 학교 코드 간소화 (Phase 8)**
- 멘토가 프로필을 등록할 때 선택하는 산업/직무 카테고리와, 담당자용(`/coordinator/mentors`)·공개(`/mentors`) 멘토 검색 화면의 필터 카테고리를 `lib/constants/categories.ts` 하나로 통일했습니다. 멘토는 각각 중복 선택(다중 선택)이 가능하고(`mentor_profiles.industry`/`job_function`이 `text[]`로 변경, `0011_mentor_category_arrays.sql`), 검색도 여러 카테고리를 동시에 체크해 OR 조건(`overlaps`)으로 찾을 수 있습니다.
- 섭외 요청 등록(`/coordinator/requests/new`)과 Q&A 산업/직무 질문(`/student/ask`)의 산업·직무 입력도 자유 텍스트 대신 같은 카테고리 목록에서 선택하도록 바꿔, 멘토 프로필과 정확히 매칭되도록 했습니다.
- 담당자·학생 회원가입의 "학교/기관 코드" 입력란을 없앴습니다. 이제 학교/기관명만 입력하면 되고, 같은 이름의 기관이 이미 있으면 재사용, 없으면 서버에서 자동으로 새 `organizations` 행을 만듭니다(내부 관리용 `org_code`는 자동 생성). 이를 위해 `organizations` 테이블에 인증 전(가입 진행 중) 상태에서도 새 행을 추가할 수 있는 RLS insert 정책을 추가했습니다(`0012_organizations_signup_insert.sql`) — `/admin/organizations`에서 관리자가 미리 기관을 등록해두는 절차는 이제 선택 사항입니다.
- 홈 화면(`/`)을 담당자(학교·기관 컨설턴트) 대상 랜딩 페이지로 새로 구성해, "조건 등록 → 카톡으로 멘토에게 전달 → 멘토 수락/거절 → 담당자 최종 선택" 흐름을 첫 화면에서 바로 설명하도록 했습니다.
