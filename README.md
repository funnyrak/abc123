...
3. `supabase/migrations/` 안의 파일을 **번호 순서대로**(0001 → 0009) SQL Editor에 붙여넣고 실행합니다. 뒤 번호일수록 앞 마이그레이션의 테이블/정책에 의존하므로 순서를 지켜야 합니다. `0009_seed_mentors.sql`까지 실행하면 실제 멘토 풀 565명이 바로 채워집니다.
